...
static int amd_navi32_reset(struct vendor_reset_dev *dev)
{
  ...
  if (ret < 0)
  {
    vr_err(dev, "amdgpu_discovery_reg_base_init failed: %d\n", ret);
    return -ENOTSUPP;
  }
  ...
  psp_bl_ready = !!(RREG32_SOC15(MP0, 0, regMP0_SMN_C2PMSG_35) & 0x80000000L);
  ...
  if (mp1_intr)
  {
    ...
    smum_send_msg_to_smc(adev, PPSMC_MSG_Mode1Reset, NULL);
  }
  ...
  if (!timeout && !(RREG32_SOC15(MP0, 0, regMP0_SMN_C2PMSG_35) & 0x80000000L))
  {
...
const struct vendor_reset_ops amd_navi32_ops =
{
  .version = {1, 1},
  .probe = amd_common_probe,
  .pre_reset = amd_common_pre_reset,
  .reset = amd_navi32_reset,
